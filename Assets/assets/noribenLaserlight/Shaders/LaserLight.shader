// 2020/6/13 Version 001 初期
// 2020/6/13 Version 002 デバッグ用コードが残っていたのを修正
// 2020/8/12 Version 002 根元部分が黒くなってしまう場合があるのを修正
Shader "Noriben/LaserLight.shader"
{
    Properties
    {
        [Header(Color)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                tex3d.x = tex3d.x * pow(tex3d.x, 4.);
                tex3d.y = tex3d.y * pow(tex3d.y, 5.);
                tex3d.z = tex3d.z * pow(tex3d.z, 5.);

                tex3d2.x = tex3d2.x * pow(tex3d2.x, 4);
                tex3d2.y = tex3d2.y * pow(tex3d2.y, 5);
                tex3d2.z = tex3d2.z * pow(tex3d2.z, 5);
                ...
                //カラー適用、コントラスト調整
                float3 col = float3(colwave,colwave,colwave) * colorGrad * clop;
                col += lensColor;
                col *= tex3d.xyz;
