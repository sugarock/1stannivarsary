...
namespace GlobalSnowEffect {
    ...
    public partial class GlobalSnow : MonoBehaviour {
                            ...
                            if (r.enabled && r.gameObject.activeInHierarchy) {
                                ...
                                if (!excludedRenderersDict.TryGetValue(r, out index)) {
                                    if (!ic.receiveSnow) {
                                        ...
                                        excludedRenderer.wasVisible = r.enabled && r.gameObject.activeInHierarchy;
        ...
        void UpdateSnowCoverageNow() {
            ...
            Shader.SetGlobalVector(ShaderParams.GlobalCamPos, new Vector4(zenithCam.transform.position.x, zenithCam.transform.position.y, zenithCam.transform.position.z, zenithCam.farClipPlane));
        }
        ...
        private void ToggleIgnoreZenithalCameraGOsLayer(bool exclude) {
            List<GlobalSnowIgnoreCoverage> ignoredGOs = instance.ignoredGOs;
            int ignoredGOsCount = ignoredGOs.Count;
            bool validFrameCount = (Time.frameCount - effectStartFrameCount) >= 5;
        ...
        private void CheckCommandBuffer() {
            if (!needRebuildCommandBuffer) {
                ...
                bool dirtyVisible = (Time.frameCount - effectStartFrameCount) < 5;
                for (int k = 0; k < excludedRenderersCount; k++) {
                    Renderer r = excludedRenderers[k].renderer;
                    if (r == null) {
