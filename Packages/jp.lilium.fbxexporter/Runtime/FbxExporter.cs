...
using Lilium.FbxExporter.Visitors;
using Lilium.FbxExporter.CustomExtensions;
...
namespace Lilium.FbxExporter
{
    ...
    public sealed class ModelExporter : System.IDisposable
    {
        ...
        private void ExportAnimationClip (AnimationClip uniAnimClip, GameObject uniRoot, FbxScene fbxScene)
        {
            ...
            // ADDED:
            if (RuntimeAnimationUtility.GetCurveBindings (uniAnimClip) != null) {
                foreach (RuntimeCurveBinding uniCurveBinding in RuntimeAnimationUtility.GetCurveBindings (uniAnimClip)) {
                    Object uniObj = uniCurveBinding.target;
                    if (!uniObj) {
                        continue;
                    }

                    AnimationCurve uniAnimCurve = uniCurveBinding.curve;
                    ...
                    // Runtime時には
                    fbxStopTime = FbxTime.FromSecondDouble (uniAnimCurve.keys[uniAnimCurve.length - 1].time);
                }
            }
            else {
#if UNITY_EDITOR
        ...
        private void ExportAnimation (GameObject uniRoot, FbxScene fbxScene)
        {
            ...
            // ADDED:
            // Try the ExportAnimationClips
            if (ExportAnimationClips.ContainsKey (uniRoot)) {
                var uniAnimClip = ExportAnimationClips[uniRoot];
                if (exportedClips.Add (uniAnimClip)) {
                    ExportAnimationClip (uniAnimClip, uniRoot, fbxScene);
                }
            }
        ...
        private bool ExportComponents (FbxScene fbxScene, bool exportAnim = true)
        {
            ...
            foreach (KeyValuePair<GameObject, FbxNode> entry in MapUnityObjectToFbxNode) {
                ...
                // try export mesh
                bool exportedMesh = false;// ExportInstance (unityGo, fbxNode);
            ...
            AnimationClip clip = null)
        {
            ...
            try {
                ...
                using (var fbxManager = FbxManager.Create ()) {
                    ...
                    // Create the exporter
                    var fbxExporter = Autodesk.Fbx.FbxExporter.Create (fbxManager, "Exporter");
        ...
        // ADDED:
        static Dictionary<Object, AnimationClip> ExportAnimationClips { get; set; } = new Dictionary<Object, AnimationClip> ();

        /// ADDED:
        /// <summary>
        /// Export single object with AnimationClip 
        /// </summary>
        /// <param name="filePath"></param>
        /// <param name="singleObject"></param>
        /// <param name="animationClip"></param>
        /// <returns></returns>
        public static string ExportObjectAndAnimation (string filePath, UnityEngine.Object singleObject, AnimationClip animationClip)
        {
            Dictionary<Object, AnimationClip> exportData = new Dictionary<Object, AnimationClip> ();
            GameObject go = ModelExporter.GetGameObject (singleObject);
            if (go) {
                exportData[go] = animationClip;
            }
            return ExportObjectsAndAnimation (filePath, new Object[] { singleObject }, animationClips: exportData);
        }
        ...
        /// </para>
        /// </summary>
        internal static string ExportObjectsAndAnimation (
            string filePath,
            UnityEngine.Object[] objects = null,
            IExportOptions exportOptions = null,
            Dictionary<GameObject, IExportData> exportData = null,
            Dictionary<Object, AnimationClip> animationClips = null
        )
        {
            LastFilePath = filePath;
            ExportAnimationClips = animationClips;
